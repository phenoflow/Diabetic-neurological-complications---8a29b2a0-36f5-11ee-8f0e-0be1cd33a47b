...
(async () => {
  ...
  await fs.appendFile('diabetic-neurological-complications-potential-cases.csv', 'patient-id,dob,codes,last-encounter\n');

  for(let patient in patients) {
    try {
      await fs.appendFile('diabetic-neurological-complications-potential-cases.csv', patient+','+dobs[patient]+',\"'+Array.from(patients[patient]).join(',')+'\",'+lastEncounter[patient].slice(0, -1)+'\n');
    } catch(error) {
      console.log(error);
    }
  }
